...
def test_weekly_shop_list_divided_by_days():
    ingredient = Ingredients.objects.create(name="unicorn")
...
def test_weekly_shop_list():
    ...
    shop_list = week.get_shop_list()
    assert shop_list == [{'measurement': 'glass', 'name': 'unicorn', 'quantity': 21.0}]

@pytest.mark.django_db
def test_weekly_shop_list_on_multiple_ingredients():
    ingredient = Ingredients.objects.create(name="unicorn")
    cream = Ingredients.objects.create(name="krem jagodowy")
    receipt = Recipe.objects.create(name="Unicorn meat")
    RecipeIngredient.objects.create(receipt=receipt, component=ingredient, measurement="glass", quantity=1)
    ...
    shop_list = week.get_shop_list()
    assert shop_list == [
        {'measurement': 'small_spoon', 'name': 'krem jagodowy', 'quantity': 42.0},
        {'measurement': 'glass', 'name': 'unicorn', 'quantity': 21.0},
    ]